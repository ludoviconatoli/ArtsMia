...
public class ArtsmiaDAO {
	...
	public void listObjects(Map<Integer, ArtObject> map) {
		...
		try {
			...
			while (res.next()) {
				//si mette una mappa all'inizio al posto di fare creare nuovi oggetti ogni volta che chiamo
				//il metodo
				//     Per evitare di fare troppe new
	...
	public List<Adiacenza> getAdiacenze() {
		...
		try {
			...
			ResultSet res = st.executeQuery();
			while(res.next()) {
				result.add(new Adiacenza(res.getInt("e1.object_id"), res.getInt("e2.object_id"), res.getInt("peso")));
